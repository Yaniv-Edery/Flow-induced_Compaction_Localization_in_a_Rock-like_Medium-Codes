...
load(fullfile(datapath, 'Workspace_strain_stress_S3_updated.mat'));
% load(fullfile(datapath, 'Workspace_full_S3.mat'),'u_original','v_original');
% load(fullfile(datapath, 'Workspace_press_disch_analyzed_S3.mat'),'P_new','Q_new');
...
place=find(round(S_av.*10^-6,6)==-0.162378); %Aproximated onset of localization (extracted manualy on the graph)
p=place(1);
classic_colors = colororder;
plot(S_av(1:point2(length(point2))).*10^-6,H11_av(1:point2(length(point2))).*10^2,'color',classic_colors(3,:),'Linewidth',2)
...
for i=places
    nexttile
    hold on
    plot(S{i}.*10^-6,H11_x{i}.*10^2,'color', classic_colors(3,:),'Linewidth',2);
    ylabel('\epsilon [%]')
...
% close all
% save('E:\Compaction_localization_data\S3\Workspace_strain_stress_analyzed_S3.mat');