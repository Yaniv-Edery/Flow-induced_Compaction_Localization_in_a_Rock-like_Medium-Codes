...
clear
close all
datapath='E:\Compaction_localization_data\S2';
load(fullfile(datapath, 'Workspace_strain_stress_S2_updated.mat'));
% load(fullfile(datapath, 'Workspace_full_S2.mat'),'u_original','v_original');
% load(fullfile(datapath, 'Workspace_press_disch_analyzed_S2.mat'),'P_new','Q_new');
...
% Dimensions
L=0.027; %length in [m]
H=0.0079; %height in [m]
sz=size(u1{1});

%% %%%%% CALCULATING THE FLUX %%%%%%%%
peak=find(P_new==max(P_new));
q=Q_new./(H*10^2*0.7);
figure
hold on
...
%% %%%%%%%%% PLOTTING STRAN-STRESS OVER THE WHOLE MEDIUM %%%%%%%%%%
point2=find(round(P_short.*10^-6,2)==0.02);
fig=figure('Units', 'normalized', 'Position', [0.2, 0.2, 0.5, 0.6]); % Wider and shorter
hold on

place=find(round(S_av.*10^-6,6)==-0.259926); %Aproximated onset of localization (extracted manualy on the graph)
p=place(1);
classic_colors = colororder;
plot(S_av(1:point2(length(point2))).*10^-6,H11_av(1:point2(length(point2))).*10^2,'color',classic_colors(2,:),'Linewidth',2)
...
for i=places
    nexttile
    hold on
    plot(S{i}.*10^-6,H11_x{i}.*10^2,'color', classic_colors(2,:),'Linewidth',2);
    ylabel('\epsilon [%]')
...
% close all
% save('E:\Compaction_localization_data\S2\Workspace_strain_stress_analyzed_S2.mat');