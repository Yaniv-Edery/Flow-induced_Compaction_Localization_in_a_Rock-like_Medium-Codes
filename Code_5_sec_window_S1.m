...
load(fullfile(datapath, 'Workspace_5_sec_window_S1_updated.mat'));
%load(fullfile(datapath, 'Workspace_5_sec_window_S1.mat'),'u_original','v_original');
...
%Average H11 over the while medium
H11_av=[];
for i=1:length(H11)
    H11_av(i)=mean(mean(H11{i},'omitnan'),'omitnan');
end

%Time vector (resolution of 0.01 [s])
Time=[0:0.01:5-0.01];

%% %%%%%%% PLOTTING %%%%%%%%%%%%
fig=figure('Units', 'normalized', 'Position', [0.2, 0.2, 0.3, 0.4]); 
hold on
classic_colors = colororder;
scatter(Time,H11_av.*10^2,'filled','MarkerFaceColor', classic_colors(1,:))
p=108; %Onset of localization
...
xlim([0 5])
set(gca,'FontSize',24)
xticks([Time(p) Time(p+200)])
xticklabels({'0', '2'})
yticks([-0.6 -0.53])
legend('','t=0','t=0.01','Location','northeast')
grid on

% saving image
%exportgraphics(fig, fullfile('E:\Compaction_localization_data\S1', '5_sec_window_S1.tiff'), 'Resolution', 600);
